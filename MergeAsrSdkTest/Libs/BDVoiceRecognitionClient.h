...
#define KToQATest
#define KDevAndDebugVer

#ifdef KDevAndDebugVer
...
// 枚举 - 调试标识定义
enum TVoiceRecognitionDebugFlags
{
    EVoiceRecognitionDebugFlagsLog = 1,                     //日志文件
    EVoiceRecognitionDebugFlagsRecordData = 2,              //录音原文件
    EVoiceRecognitionDebugFlagsPreSoundData = 4,            //本地处理后的录音文件
    //所有日志打开
    EVoiceRecognitionDebugFlagsAll = (EVoiceRecognitionDebugFlagsLog | EVoiceRecognitionDebugFlagsRecordData | EVoiceRecognitionDebugFlagsPreSoundData)
};

// @protocol - MBDAutoTestTextViewLogDelegate
// @brief - 自动化测试状态通知
@protocol MBDAutoTestTextViewLogDelegate<NSObject>
@optional

- (void)cleanLog;

- (void)logOut:(NSString*)aLog;

- (void)cleanResultLog;

- (void)cleanResultLogAfter:(int)length;

- (void)logOutToResult:(NSString*)aLog;

@end // MBDAutoTestTextViewLogDelegate
...
// 设置在线超时时间, 超时后同步使用离线合成
- (void)setTimeoutForOnlineService:(NSTimeInterval)time;
